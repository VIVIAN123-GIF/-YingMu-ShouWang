...
现有授权素材只能证明负向/低质量分支保持保守，不能生成可信的真实像素 ORANGE 正向报告。
下一步需要同一授权参与者、同一固定机位的两段素材：

1. 风险段：有效坐站转换后出现至少两个独立且清晰的不稳信号；
2. 恢复段：姿态稳定并满足恢复观察窗口。

拍摄前固定标签、动作时间点、授权记录和保留期限，禁止根据算法结果反向修改标签或降低阈值。

### 5.2 实机场景尚未最终校准

发布包只携带结构正确的脱敏示例多边形。最终机位确定后必须本地标定危险区、支撑区、障碍区和安全区，
并验证人体脚点与场景坐标一致。未完成标定时应显示场景上下文不足。
...
拍摄人员必须保证安全，不实施真实跌倒。两段素材都要补齐参与者匿名引用、授权、日期、机位、有效性、
拍摄时间和保留期限。先完成人工双人标注，再运行算法。
...
只有报告同时包含 ORANGE、Agent Job、模拟干预、OBSERVING、RESOLVED 和 POST 快照，且引用完整，
才能把真实像素工程正向闭环写为通过。
