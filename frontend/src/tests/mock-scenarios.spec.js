...
describe('固定 JSON 演示闭环', () => {
  ...
  it('三类受控场景分别关联独立的授权回放素材', () => {
    ...
    expect(byId.get('asset-fall-authorized').fallback_url).toBe('/media/selected/new-risk-left-take03.mp4')
    expect(byId.get('asset-mental-week').fallback_url).toBe('/media/selected/activity-route-a-b-c-01.mp4')
    expect(byId.get('asset-green-daily').fallback_url).toBe('/media/selected/new-normal-control-take02.mp4')
