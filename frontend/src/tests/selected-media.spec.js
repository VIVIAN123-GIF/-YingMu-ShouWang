...
describe('精选受控视频映射', () => {
  it('保留 28 条脱敏的本地精选素材，且全部标记为回放模拟数据', () => {
    expect(selectedMedia).toHaveLength(28)
  ...
  it('仅向显式映射的模拟回放事件提供工程对照，真实设备与无关领域不替换媒体', () => {
    ...
    expect(selected.primary_asset_id).toBe('selected-p01-golden-loop-01')
    ...
    expect(getSelectedEventMedia({ event_id: 'event-fi-resident-001-run-01-event-green-daily', source_mode: 'RECORDED_REPLAY', simulated: true })?.primary_asset_id)
      .toBe('selected-p03-neg-normal-rise-walk-01')
