...
describe('Mock 授权视频素材', () => {
  ...
  it('未配置覆盖地址时使用固定 JSON 中的跌倒风险片段', async () => {
    ...
    expect(asset).toMatchObject({
      fallback_url: '/media/selected/new-risk-left-take03.mp4',
      available: true,
      verification_status: 'AUTHORIZED_LOCAL_CLIP',
    })
  })
  ...
  it('为活动轨迹和日常基线返回各自的回放片段', async () => {
    ...
    expect(daily.fallback_url).toBe('/media/selected/new-normal-control-take02.mp4')
  ...
  it('自动模式遇到资产接口500时切换到对应授权回放', async () => {
    ...
    expect(asset.fallback_url).toBe('/media/selected/new-normal-control-take02.mp4')
