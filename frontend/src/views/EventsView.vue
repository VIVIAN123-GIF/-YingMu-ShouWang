...
import { alarmIdentifierLabel, deviceReferenceLabel, displayValueLabel, domainLabel, evidenceTypeLabel, formatDateTime, formatRiskScore, ruleLabel, rulesetVersionLabel, statusLabel } from '../utils/format'
...
const filteredEvents = computed(() => events.value
  ...
  .sort((left, right) => {
    const liveDifference = Number(right.source_mode === 'LIVE_DEVICE' && right.simulated === false)
      - Number(left.source_mode === 'LIVE_DEVICE' && left.simulated === false)
...
const latestLiveEventId = computed(() => events.value
  .filter((event) => event.source_mode === 'LIVE_DEVICE' && event.simulated === false)
  .sort((left, right) => new Date(right.created_at) - new Date(left.created_at))[0]?.event_id || '')
...
function scheduleAlarmPolling() {
  stopAlarmPolling()
  if (!alarmPollingActive || runtime.mode === 'replay') return
  alarmTimer = window.setTimeout(async () => {
    alarmTimer = null
    if (alarmPollInFlight) { scheduleAlarmPolling(); return }
    alarmPollInFlight = true
    try { await Promise.all([load(true), loadAlarmTasks()]) }
    finally { alarmPollInFlight = false; scheduleAlarmPolling() }
  }, 2000)
}
...
onMounted(async () => { alarmPollingActive = true; await Promise.all([load(), loadAlarmTasks()]); scheduleAlarmPolling() })
watch(() => runtime.mode, async () => { await Promise.all([load(), loadAlarmTasks()]); scheduleAlarmPolling() })
onBeforeUnmount(() => { alarmPollingActive = false; stopAlarmPolling() })
</script>

<template>
  <div v-loading="loading" data-testid="events-view">
    <PageHeader title="风险事件与处置" description="按时间查看风险识别、分级预警、家属反馈和最终处置结果。">
      ...
      >
        ...
        <button type="button" class="timeline-event-card" @click="router.push(`/events/${event.event_id}`)">
          <div class="timeline-event-heading">
            <div><span class="event-domain">{{ domainLabel(event.primary_domain) }}</span><h2>{{ event.title }}</h2><el-tag v-if="event.event_id === latestLiveEventId" type="success" effect="plain" size="small">最新真实运行</el-tag></div>
          ...
          <div class="timeline-event-footer">
            ...
            <span>{{ rulesetVersionLabel(event.ruleset_version) }}</span>
    ...
    <TechnicalDisclosure
      title="工程复核与设备任务"
      :summary="`${reviews.length} 条需要复核的评估 · ${alarmTasks.length} 条设备任务`"
    >
      <section class="content-card alarm-processing-card" data-testid="risk-reviews">
        <div class="card-heading">
          <div><span class="section-kicker">规则复核</span><h2>需要复核的评估</h2></div>
        ...
        <div class="alarm-task-list">
          <article v-for="review in reviews" :key="review.trace_id" class="alarm-task-row">
            ...
            <div><span>{{ formatDateTime(review.evaluated_at) }}</span><span>{{ ruleLabel(review.matched_rule) }}</span><span>{{ review.ruleset_version ? rulesetVersionLabel(review.ruleset_version) : '规则版本未提供' }}</span></div>
      ...
      <section class="content-card alarm-processing-card" data-testid="alarm-processing">
        ...
        <div v-loading="alarmLoading" class="alarm-task-list">
          <article v-for="task in alarmTasks" :key="task.task_id" class="alarm-task-row">
            <div><strong>{{ alarmIdentifierLabel(task.alarm_ref) }}</strong><span>当前居民 · {{ deviceReferenceLabel(task.device_ref) }}</span></div>
